 ...
 * @author tim
 */
public class CycleCanvas extends JComponent implements ResetListener,
        SurfaceChangeListener, PathSelectionListener, PathChangeListener,
        RegionListener, MouseInputListener, KeyListener {
    static final long serialVersionUID = 1L;
    static final int SNAP_DIST = 10;
    static final Stroke ACTIVE_STROKE = new BasicStroke(2.0f);
    static final Stroke VISIBLE_STROKE = new BasicStroke(2.0f,
    ...
    public CycleCanvas(PicState picState) {
        ...
        // We listen to both changes to the underlying surface and the imortant
        // paths
    ...
    /**
     * draws a black dot at the given (screen) coordinates. Used for nodes on
     * paths
     * 
     * @param pt
     * @param g
    ...
    private void paintCuts(Graphics2D g) {
        ...
        Collection<Line2D> cuts = picState.getCutScheme().cutGraph(minClip,
                maxClip);
    ...
    private void paintPaths(Graphics2D g) {
        ...
        for (RiemannPath path : visiblePaths) {
            ...
            for (SheetedSeg seg : path.getSegments(picState.getCutScheme())) {
                ...
                g.setColor(Colours.SHEET_COLOURS[seg.sheet
                        % Colours.SHEET_COLOURS.length]);
    ...
    private void paintActivity(Graphics2D g) {
        ...
        if (selectedPoint != null) {
            ...
            g.drawRect((int) pos.getX() - 5, (int) pos.getY() - 5, 10, 10);
        }
    ...
    /**
     * Just draw an arrowhead in a given direction. Why doesn't java include
     * this?
     */
    private void drawArrow(Graphics2D g, Point2D head, Point2D dir) {
        ...
        init = new Point2D.Double(head.getX() + len * (sT - cT), head.getY()
                + len * (cT + sT));
        g.draw(new Line2D.Double(init, head));
    }
    ...
    /**
     * Setup the coordinate transformations so that the bits of the complex
     * plane we want are visible.
     */
    void setViewport() {
        mathsToJava.setToIdentity();
        mathsToJava.scale(this.getWidth() / (ur.getX() - ll.getX()),
                -this.getHeight() / (ur.getY() - ll.getY()));
    ...
    /**
     * Right click cancels drawing but does nothing else. Otherwise we
     * start/continue drawing as necessary.
     */
    @Override
    ...
    /**
     * If we're already dragging a node carry on. If not, start dragging if
     * appropriate
     * 
     */
    @Override
    ...
    /**
     * Snap a given point to the closest node on the active path if appropriate.
     * Depends on viewport
     * 
     * @param pt
     *            java coordinates, the actual mouse event point.
     * @param excluding
     *            . A point in the activePath (so maths coords) to ignore
    ...
    /**
     * Similar to snapToActive, but returns null if we're outside snap distance
     * so caller easily knows not to bother snapping
     */
    ...
    private void fireSelectedPointChanged(Point2D pt, boolean rapid) {
        ...
        for (int i = ls.length - 2; i >= 0; i -= 2) {
            if (ls[i] == SelectedPointListener.class) {
                ((SelectedPointListener) ls[i + 1]).selectedPointChanged(pt,
                        rapid);
            }
        }
    }
