...
public class RadialCutScheme extends CutScheme {
    ...
    @Override
    public java.util.List<SheetedSeg> splitSegment(Point2D begin, Point2D end,
            int sheetNum) {
    ...
    void precalcPermutations() {
        try {
            ...
            s = String.format("cutmono := mono_to_shift(mono[3], %s, %s):",
                    MapleUtils.pointToString(surface.getBasePoint()),
                    MapleUtils.pointToString(surface.getSheetsBase()));
            maple.evaluate(s);
            ...
            cutsPermutation = new HashMap<Point2D, Algebraic>();
            System.err
                    .println("Unexpected error converting canonical sheets to permutation "
                            + "for each cut.");
            System.err.println(e);
        }
    ...
    /**
     * Work out how crossing a given branch cut in a given direction changes the
     * sheet.
     * 
     * @param sheet
     *            Sheet before the branch cut
     * @param descr
     *            Description of the branch cut and direction we're crossing.
    ...
    int shiftSheets(int sheet, SheetChange descr) {
        try {
            ...
            System.err
                    .println("Unable to calculate sheet change across a single, known cut");
     ...
     * @return nth entry is y-value on sheet n.
     */
    @Override
    public ArrayList<Point2D> getAllSheets(Point2D pt)
            throws SheetPropagationException {
        ...
        try {
            mapSheets = maple.propagateSheets(surface.getCurveString(),
                    mapSheets, from, pt);

            // Now we have to apply the sheet changes of any cuts we've crossed
            // between
            // from and to.
            ...
            System.err
                    .println("Couldn't update sheet order by analytic continuation");
    ...
    public Collection<Line2D> cutGraph(Point2D minClip, Point2D maxClip) {
        ...
        Collection<Line2D> dispCuts = new LinkedList<Line2D>(
                branchCuts.values());
        Point2D base = surface.getBasePoint();
        dispCuts.add(new Line2D.Double(base, new Point2D.Double(minClip.getX(),
                base.getY())));
    ...
    Collection<SheetChange> segmentParts(Point2D begin, Point2D end) {
        ...
        if (surface.hasInfiniteBranch()) {
            ...
            Line2D cut = new Line2D.Double(basePoint, new Point2D.Double(
                    Double.NEGATIVE_INFINITY, basePoint.getY()));
            if (cut.intersectsLine(segment)) {
                // Special code here since infinities are involved.
                double t = (basePoint.getY() - begin.getY())
                        / (end.getY() - begin.getY());
                double x = begin.getX() + t * (end.getX() - begin.getX());
                Point2D isection = new Point2D.Double(x, basePoint.getY());

                SheetChange sc = new SheetChange(RiemannSurface.INFINITY,
                        isection, begin.getY() > basePoint.getY() ? -1 : 1);
