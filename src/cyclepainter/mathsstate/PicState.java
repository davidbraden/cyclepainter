...
public class PicState {
    ...
    public PicState(InputStream data) throws IOException, PicFormatException,
    ...
    /**
     * Resets all data to a primitive elliptic curve with a couple of empty
     * cycles.
     **/
    ...
    public void writeData(OutputStream outBasic) {
        ...
        out.printf("mono_base := %s:\n",
                MapleUtils.pointToString(surface.basePoint));
        out.printf("sheets_base := %s:\n",
                MapleUtils.pointToString(surface.sheetsBase));
        out.println();
        ...
        while (i.hasNext()) {
            ...
            sheetList.append(MapleUtils.pointToString(sheet));
    ...
    public void writeMetapost(OutputStream outBasic) {
        ...
        for (RiemannPath path : paths.values()) {
            ...
            // Now work out where we need to split and do that. First seg gets
            // an arrow.
            int curT = 0;
        ...
        for (Point2D branch : surface.getFiniteBranches()) {
            if (branch.getX() < minX)
                minX = branch.getX();
            out.printf(
                    "\tdraw ( (%f,%f)--(%f,%f) ) transformed trans withcolor 0.5white;\n",
                    surface.getBasePoint().getX(), surface.getBasePoint()
                            .getY(), branch.getX(), branch.getY());
        }
        ...
        out.printf(
                "\tdraw ( (%f,%f)--(%f,%f) ) transformed trans withcolor 0.5white;\n",
                surface.getBasePoint().getX(), surface.getBasePoint().getY(),
     ...
     */
    public void readData(InputStream inBasic) throws IOException,
            PicFormatException, SetSurfaceException {
        ...
        try {
            ...
            throw new PicFormatException(
                    "Maple could not interpret .pic file: " + e);
        }
        ...
        try {
            ...
            throw new PicFormatException(
                    "Incomplete .pic file: should define: f, x, y");
        }
        ...
        try {
            ...
            throw new PicFormatException(
                    "Incomplete .pic file: path data error" + e);
        }
        fireResetEvent();
        firePathSetEvent();
    }
